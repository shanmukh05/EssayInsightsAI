...
def train_model(essay_df, config, label_info, output_folder):
    ...
    if os.path.exists(model_path):
        config_model = AutoConfig.from_pretrained(config["paths"]["model"]+"/config.json") 
        config_model.num_labels = num_classes
        config_model.id2label = id2label
        config_model.label2id = label2id
        model = AutoModelForTokenClassification.from_pretrained(
                            config["paths"]["model"],config=config_model, 
                        )   
    else:
        model = AutoModelForTokenClassification.from_pretrained(
                            config["paths"]["model"],
                            label2id=label2id, id2label=id2label, num_labels=num_classes
                        )   
...
def load_saved_model(config, label_info, ckpt_path, output_folder):
    ...
    network = FeebackPrizeNetwork.load_from_checkpoint(ckpt_path, model=model, config=config, num_classes=num_classes)
